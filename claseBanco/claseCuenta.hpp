...
public:
    ...
    explicit Cuenta(std::string nombre="\0", double monto=0.0);
    virtual ~Cuenta();
    void setTitular(std::string nombre);
...
public:
    cCorriente(std::string nombre="\0", double monto=0.0, double limite=0.0);
    virtual ~cCorriente();
...
private:
    int extracciones;
public:
    cAhorro(std::string nombre="\0", double monto=0.0);
    virtual ~cAhorro();
    virtual void extraer(double);
...
public:
    cUniversitaria(std::string nombre="\0", double monto=0.0);
